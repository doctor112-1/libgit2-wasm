...
static int config_level_to_sysdir(int *out, int config_level)
{
	...
	git_error_set(
	        GIT_ERROR_INVALID, "invalid config path selector %d",
	        config_level);
	return -1;
}
...
{
	return git__user_agent_product ? git__user_agent_product : "git/2.0";
}

const char *git_settings__user_agent(void)
{
	return git__user_agent ? git__user_agent : "libgit2 " LIBGIT2_VERSION;
}
...
int git_libgit2_opts(int key, ...)
{
	...
	case GIT_OPT_SET_SEARCH_PATH: {
		...
		if ((error = config_level_to_sysdir(&level, va_arg(ap, int))) >=
		    0)
	...
	case GIT_OPT_SET_TEMPLATE_PATH:
		error = git_sysdir_set(
		        GIT_SYSDIR_TEMPLATE, va_arg(ap, const char *));
		break;
...
#if defined(GIT_HTTPS_OPENSSL) || defined(GIT_HTTPS_OPENSSL_DYNAMIC)
	{
		...
		git_error_set(
		        GIT_ERROR_SSL,
		        "TLS backend doesn't support adding of the raw certs");
		error = -1;
#endif
	break;
	...
	case GIT_OPT_GET_USER_AGENT: {
		...
		if ((error = git_buf_tostr(&str, out)) < 0 ||
		    (error = git_str_puts(&str, git_settings__user_agent())) <
		            0)
			break;
	...
	case GIT_OPT_GET_USER_AGENT_PRODUCT: {
		...
		if ((error = git_buf_tostr(&str, out)) < 0 ||
		    (error = git_str_puts(
		             &str, git_settings__user_agent_product())) < 0)
			break;
	...
	case GIT_OPT_SET_HOMEDIR:
		error = git_sysdir_set(
		        GIT_SYSDIR_HOME, va_arg(ap, const char *));
		break;
	...
	case GIT_OPT_SET_SERVER_CONNECT_TIMEOUT: {
		...
		if (timeout < 0) {
			git_error_set(
			        GIT_ERROR_INVALID, "invalid connect timeout");
			error = -1;
		} else {
